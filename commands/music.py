...
class Music(commands.Cog):
    ...
    @commands.command()
    async def queue(self, ctx):  # displays the current guilds queue
        if len(self.song_queue[ctx.guild.id]) == 0:
    ...
    async def skip(self, ctx):
        ...
        if not skip:
            embed = discord.Embed(
                title="Skip Failed", description="*Voting to skip the current song has failed.*\n\n**Voting failed, the vote requires at least 80% of the members to skip.**", colour=discord.Colour.purple())
